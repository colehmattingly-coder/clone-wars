...
function placeholderBackground(ctx, width, height, time) { ctx.fillStyle = '#78c8ef'; ctx.fillRect(0, 0, width, height); }
function placeholderGround(ctx, width, height, groundHeight, offset) { ctx.fillStyle = '#61a33b'; ctx.fillRect(0, height - groundHeight, width, groundHeight); }
function placeholderBird(ctx, x, y, size, velocity) { ctx.fillStyle = '#ffd166'; ctx.fillRect(x - size / 2, y - size / 2, size, size); }
function placeholderPipe(ctx, x, gapTop, gapBottom, pipeWidth, height) { ctx.fillStyle = '#4f8b3b'; ctx.fillRect(x, 0, pipeWidth, gapTop); ctx.fillRect(x, gapBottom, pipeWidth, height - gapBottom); }
...
function startGame() {
  ...
  pipesMade = 0;
  groundOffset = 0;
  lastGapTop = (CONFIG.canvasHeight - CONFIG.groundHeight) / 2 - CONFIG.pipeGap / 2;
  score = checkpoint;
  streak = 0;
  secondsSinceCrash = 0;
  overlay.hidden = true;
...
function drawFlame() {
  if (CONFIG.fix !== 'custom' || streak < 1) return;
  const intensity = Math.min(1, streak / 8);
  ctx.save();
  ctx.fillStyle = '#ff6b35';
  ctx.beginPath();
  ctx.moveTo(CONFIG.canvasWidth / 4 - 8, y + CONFIG.birdSize / 2);
  ctx.lineTo(CONFIG.canvasWidth / 4, y + CONFIG.birdSize / 2 + 9 + intensity * 8);
  ctx.lineTo(CONFIG.canvasWidth / 4 + 8, y + CONFIG.birdSize / 2);
  ctx.closePath();
  ctx.fill();
  ctx.restore();
}
function frame(now) {
  const previous = frame.last || now;
  ...
  if (state === 'playing') {
    ...
    const customSpeed = CONFIG.fix === 'custom' ? pipeSpeed * (1 + Math.min(streak, 10) * 0.08) : pipeSpeed;
    const customGap = CONFIG.fix === 'custom' ? Math.max(90, pipeGap - Math.min(streak, 10) * 5) : pipeGap;
    const birdX = CONFIG.canvasWidth / 4;
    groundOffset += pipeSpeed * seconds;
    if (pipes.length === 0) addPipe(customGap);
    else if (pipes[pipes.length - 1].x <= CONFIG.canvasWidth - CONFIG.pipeSpacing) addPipe(customGap);
    for (const pipe of pipes) {
      pipe.x -= customSpeed * seconds;
      if (!pipe.scored && pipe.x + CONFIG.pipeWidth < birdX) {
        pipe.scored = true;
        score += 1;
        if (CONFIG.fix === 'custom') { streak += 1; score += streak >= 3 ? 2 : 1; }
        play('score');
      }
    }
    ...
    const hitsPipe = pipes.some((pipe) => birdRight > pipe.x && birdLeft < pipe.x + CONFIG.pipeWidth && (birdTop < pipe.gapTop || birdBottom > pipe.gapBottom));
    if (birdBottom >= CONFIG.canvasHeight - CONFIG.groundHeight || birdTop <= 0 || hitsPipe) crash();
  ...
  if (state === 'playing') {
    ctx.save();
    ctx.font = 'bold 42px system-ui, sans-serif'; ctx.textAlign = 'center'; ctx.lineWidth = 5; ctx.strokeStyle = '#172033'; ctx.fillStyle = '#fff';
    ctx.strokeText(String(score), CONFIG.canvasWidth / 2, 62); ctx.fillText(String(score), CONFIG.canvasWidth / 2, 62); ctx.restore();
  }
